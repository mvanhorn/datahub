...
class ParseTask:
    ...
    def __post_init__(self) -> None:
        ...
        if self.key is not None and not isinstance(self.key, (str, int, tuple)):
            raise TypeError(
                f"ParseTask.key must be None, str, int, or tuple (got "
                f"{type(self.key).__name__}); it must be picklable to cross the "
                f"worker process boundary."
            )
...
class ParseOutcome:
    ...
    The invariant is enforced structurally: ``result`` and ``error`` are never
    both set. Callers should test :attr:`failed` (or :attr:`ok`) rather than
    re-checking the two fields by hand.
    """
    ...
    def failed(self) -> bool:
        """True if the worker did not produce a usable result (it raised/died, or
        no result was produced). Normal parse failures captured inside
        ``result.debug_info`` are NOT failures at this level — the worker ran to
        completion, so ``failed`` is False."""
