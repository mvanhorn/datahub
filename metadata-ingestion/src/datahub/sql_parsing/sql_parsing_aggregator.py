...
class SqlParsingAggregator(Closeable):
    ...
    def _add_to_query_map(
        ...
        if query_fingerprint in self._query_map:
            ...
            # Determine which record owns the later timestamp *before* overwriting it,
            # so actor/session_id track the same winner and are stable across merge orders.
            _new_ts_wins = new.latest_timestamp is not None and (
                current.latest_timestamp is None
                or new.latest_timestamp > current.latest_timestamp
            )
            ...
            # Adopt actor from whichever record has the later timestamp so that actor
            # is tied to the same winner as latest_timestamp and is stable across
            # merge orders.
            if _new_ts_wins:
