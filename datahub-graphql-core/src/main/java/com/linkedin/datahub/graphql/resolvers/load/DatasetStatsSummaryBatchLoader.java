...
public class DatasetStatsSummaryBatchLoader {
  ...
  // Summary values only change when new usage stats are ingested (daily), so a 1-day TTL matches
  // the per-URN UsageClientCache default (CACHE_CLIENT_USAGE_CLIENT_TTL_SECONDS). Bounded so the
  // cache cannot grow without limit across the dataset population.
  private static final Duration CACHE_TTL = Duration.ofHours(24);
  private static final long CACHE_MAX_SIZE = 50_000;

  private final TimeseriesAspectService timeseriesAspectService;
  // Shared across per-request DataLoader instances (one cache per engine). The DatasetStatsSummary
  // values are viewer-independent — per-URN authorization is enforced (below) before any URN's
  // data is read from the cache or ES — so keying on Urn alone is safe.
  ...
  /** Builds the shared summary cache. Construct once (per engine) and reuse across requests. */
  ...
  public static DataLoader<Urn, DatasetStatsSummary> createDataLoader(
      ...
      final Cache<Urn, DatasetStatsSummary> cache,
